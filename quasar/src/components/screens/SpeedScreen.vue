  ...
  <div class="wrap"><div class="screen">
    ...
    <div class="speed-widget">
      ...
      <div class="speed-icons">
        <span v-for="(o, k) in options" :key="k" class="speed-ic" :class="{ on: val === k }" v-html="duo(o.icon)" />
      </div>
      <input type="range" class="rng" min="0" max="2" step="1" v-model.number="val">
      <div class="speed-labels">
        <span v-for="(o, k) in options" :key="k" :class="{ on: val === k }">{{ f.T(o.label) }}</span>
      </div>
...
const val = ref(prev ? options.findIndex(o => o.label[0] === prev.t[0]) : 1)
function go () {
